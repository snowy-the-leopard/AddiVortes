...
\\\\
The code used to generate the benchmarking data, as well as the generated data itself and all referenced files, are available to view at \url{https://github.com/snowy-the-leopard/AddiVortes}. This data was generated using the \textsc{AddiVortes} R package, version 0.6.6, available to view here \url{https://github.com/johnpaulgosling/AddiVortes}, and the \textsc{addivortes} Python package, version 0.6.6, available to view here \url{https://github.com/johnpaulgosling/py-addivortes}.
...
The full benchmarks, which carry out model fitting and prediction on the above 4 datasets, were run for 500 iterations each. The code can be found at \nolinkurl{AddiVortes\ tests.R} and \nolinkurl{AddiVortes\ tests.py}
...
The mean fit time using Python was substantially lower for all models. With an average decrease of 9\% in mean fit time using Python, we can certainly conclude that the Python implementation is significantly faster than R at fitting the AddiVortes model using a Euclidean metric.
The standard deviation of fit times using Python gave mixed results when compared to using R, but for the first 3 models Python has much higher standard deviations for fit time.
\\
\\
Hence, Python produces lower but less consistent fit times than R.
\subsection{Prediction times}
The mean prediction time using Python was significantly lower for all models, with an average decrease of 38\% when using Python instead of R. The standard deviation of prediction times using Python was significantly lower for all models, with an average decrease of 35\% when using Python instead of R.
\\
\\
Therefore in terms of both lower and more consistent prediction times, Python is superior.
\subsection{In-sample RMSEs}
The mean in-sample RMSE using Python was slightly higher for all models, with an average increase of 2\% when using Python instead of R. The standard deviation of in-sample RMSEs using Python were on average 4\% higher than when using R.
\\
\\
Therefore it is unlikely there is a significant difference between in-sample RMSEs between R and Python.
\subsection{Out-of-sample RMSEs}
The mean out-of-sample RMSE using Python consistently gave slightly higher values, with an average increase of 1\% across the 4 models. The standard deviation of out-of-sample RMSEs using Python gave mixed results, but overall seemed to be higher, with an average increase of 3\% across the 4 models.
\\
\\
Therefore it is unlikely there is a significant difference between out-of-sample RMSEs between R and Python
\newpage
...
$$\\
Note that we are using samples with 500 observations and hence need to use our standard errors, by scaling our standard deviations from tables 2 and 4 by a factor of $\frac{1}{\sqrt{n}}$. We can then calculate the standard error of the difference of the means:
$$
...
\end{table}
\newpage
Clearly, the vast majority of the differences in fit times and prediction times are highly significant, with Python having the significantly lower times for both. Some of the iRMSE and oRMSE differences are significant, but there are also some insignificant results, especially within oRMSE where significances were lower than in iRMSE.\\
\\
Therefore, since there is limited evidence to suggest the R implementation has lower oRMSEs, and there is strong evidence to suggest the Python implementation has faster fit and prediction times, I would suggest Python to be used for further implementation and investigation
\end{document}