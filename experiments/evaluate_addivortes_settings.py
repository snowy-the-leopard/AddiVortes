...
X_TRAIN = PARENT / "benchmarks" / "datasets" / "boston" / "x_train.csv"
X_TEST = PARENT / "benchmarks" / "datasets" / "boston" / "x_test.csv"
Y_TRAIN = PARENT / "benchmarks" / "datasets" / "boston" / "y_train.csv"
Y_TEST = PARENT / "benchmarks" / "datasets" / "boston" / "y_test.csv"
TARGET_COL = "x"
...
def evaluate_setting(index_and_settings: tuple[int, dict]) -> tuple[int, dict]:
    ...
    model = AddiVortesRegressor(
        ...
        burn_in=int(round(int(settings["mcmcIter"]) *float(settings["mcmcBurnin"]))),
        nu=int(settings["nu"]),
        q=float(settings["q"]),
