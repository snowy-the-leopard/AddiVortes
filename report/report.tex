...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../screening/outputs/fit_time/final_effects.png}
        \caption{Fit time}
        \label{fig:fit}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../screening/outputs/pred_time/final_effects.png}
    ...
    \label{fig:grid}
\end{figure}
We will investigate the local effects of changing these parameters later on, and we can use the results of this screening to shape further investigation, and to determine if we can generalise results we obtain later.
...
I wrote code to generate random subsets of the Earthquake dataset of sizes $n=100, 200, \cdots, 10000$. In order to obtain a broader picture of the trends as we vary $n$, each size, e.g. $n=100$ was generated independently $k$ times, taking the maximum possible value of $k$ such that $nk\leq 100000$, to bound the time needed to generate and work on subsets of any given size, and make each batch size a suitable unit process for multiprocessing. This was implemented to minimise the random noise in the metrics as we take varying sub-populations of the full data. This leads to us taking $k = \lfloor \frac{100000}{n}\rfloor$. The code used to generate the subsets, as well as the subsets themselves for reproducibility, are available at \nolinkurl{AddiVortes\ effects \ n}\cite{repo}

Once the subsets were generated, an AddiVortes fit was carried out on each subset, and the median statistics over each batch size were calculated. The results of which can be found at \nolinkurl{AddiVortes\ effects\ n\ addivortes_results.csv}. Graphs of the result summaries are below:
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../effects/n/graphs/fit.jpg}
        \caption{Median fit time}
        \label{fig:fit}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../effects/n/graphs/pred.jpg}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../effects/n/graphs/iRMSE.jpg}
        \caption{In-sample RMSE}
        \label{fig:irmse}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../effects/n/graphs/oRMSE.jpg}
        \caption{Out-of-sample RMSE}
        \label{fig:ormse}
    \end{subfigure}
    
    \caption{Performance metrics as a function of subset size $n$.}
    \label{fig:grid}
\end{figure}
